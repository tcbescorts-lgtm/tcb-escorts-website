...
js/script.js        Nav, forms, FAQ accordion, counters
...
2. Open `js/script.js` is NOT where the key goes — the key goes in each HTML file. In `contact.html` and `careers.html`, find:
   ```html
...
Every page is plain HTML — open any `.html` file and edit the text directly. Shared styles live in `css/style.css`; shared behavior (nav, forms, FAQ) lives in `js/script.js`. There's no template engine, so header/footer markup is repeated on each page — if you change the nav or footer, update it on all six pages.
